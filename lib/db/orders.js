...
const resolvePaymentRecord = (payment = {}, fulfillment, total) => {
  ...
  if (payment.mode === 'card_now') {
    // Si un PaymentIntent Stripe est fourni, le paiement est déjà confirmé côté client
    // (y compris après authentification 3D Secure si nécessaire)
    if (payment.stripePaymentIntentId) {
      return {
        method: 'card_online',
        status: 'paid', // Paiement déjà confirmé côté client (avec 3D Secure si nécessaire)
        amount: total,
        processor: 'stripe',
        processor_id: payment.stripePaymentIntentId,
        // Stocker aussi le PaymentMethod ID pour référence
        metadata: payment.stripePaymentMethodId ? { payment_method_id: payment.stripePaymentMethodId } : null,
      }
    }
    // Sinon, ancien système simulé (pour compatibilité)
...
export async function createOrder(slug, payload) {
  ...
  if (paymentRecord) {
    const paymentInsert = {
      ...
      processor: paymentRecord.processor,
      processor_id: paymentRecord.processor_id,
      // Stocker le PaymentMethod ID dans auth_code pour confirmation ultérieure
      auth_code: paymentRecord.metadata?.payment_method_id || null,
    }
...
export async function advanceOrderStatus(orderId) {
  console.log(`[advanceOrderStatus] Appelé pour la commande: ${orderId}`)
  
  if (!orderId || typeof orderId !== 'string') {
    throw new OrderValidationError('Identifiant de commande requis.', 400)
  }

  // Récupérer les informations complètes de la commande pour les notifications SMS
  ...
  // Si on passe de "received" à "preparing", vérifier que le paiement est confirmé
  // Note: Le paiement est maintenant confirmé côté client avant la création de la commande
  // (y compris avec gestion du 3D Secure), donc on vérifie juste qu'il est bien payé
  if (order.status === 'received' && nextStatus === 'preparing') {
    // Vérifier s'il y a un paiement Stripe
    const { data: payment, error: paymentError } = await supabaseServer
      .from('payments')
      .select('id, processor_id, status')
      .eq('order_id', orderId)
      .eq('processor', 'stripe')
      .maybeSingle()
    ...
    // Si un paiement Stripe existe et n'est pas encore payé, c'est une erreur
    // (le paiement devrait déjà être confirmé côté client avec gestion 3D Secure)
    if (payment && payment.status !== 'paid') {
      // Le paiement devrait déjà être confirmé côté client
      // Si ce n'est pas le cas, c'est une erreur - annuler la commande
      const { error: cancelError } = await supabaseServer
        .from('orders')
        .update({
          status: 'cancelled',
          cancelled_at: new Date().toISOString(),
          failure_reason: 'Paiement non confirmé - erreur système',
        })
        .eq('id', orderId)
      ...
      throw new OrderValidationError(
        'Le paiement n\'a pas été confirmé. La commande a été annulée.',
        402
      )
    }
  }
  ...
  // Envoyer des notifications SMS selon le type de service et le statut
  // On fait cela de manière asynchrone pour ne pas bloquer la mise à jour du statut
  console.log(`[Order Status] Transition: ${order.status} -> ${nextStatus}, Fulfillment: ${order.fulfillment}`)
  sendOrderStatusSMS(order, nextStatus)
    .then((result) => {
      if (result?.success) {
        console.log(`[Order Status] SMS envoyé avec succès pour la commande ${orderId}, statut: ${nextStatus}`)
      } else {
        console.warn(`[Order Status] SMS non envoyé pour la commande ${orderId}:`, result?.reason || 'Aucun message configuré pour cette transition')
      }
    })
    .catch((smsError) => {
      // Logger l'erreur mais ne pas bloquer le processus
      console.error('[Order Status] Erreur lors de l\'envoi du SMS:', smsError)
    })
...
/**
 * Send SMS notification based on order status transition
 * @param {Object} order - Order object with fulfillment, status, phone info, etc.
 * @param {string} newStatus - The new status the order is transitioning to
 */
async function sendOrderStatusSMS(order, newStatus) {
  try {
    // Import SMS helper dynamically to avoid circular dependencies
    const { sendSMS, formatPhoneForSMS } = await import('../sms/send')

    // Déterminer le numéro de téléphone du client
    let customerPhone = null
    if (order.fulfillment === 'pickup') {
      customerPhone = order.pickup_phone
      console.log(`[Order Status SMS] Pickup - Numéro: ${customerPhone || 'NON DISPONIBLE'}`)
    } else if (order.fulfillment === 'delivery') {
      customerPhone = order.customer?.phone
      console.log(`[Order Status SMS] Delivery - Numéro client: ${customerPhone || 'NON DISPONIBLE'}, customer_id: ${order.customer_id}`)
    }

    if (!customerPhone) {
      // Pas de numéro de téléphone disponible, on ne peut pas envoyer de SMS
      console.warn(`[Order Status SMS] Pas de numéro de téléphone disponible pour la commande ${order.id}, fulfillment: ${order.fulfillment}`)
      return { success: false, reason: 'No phone number available' }
    }

    const formattedPhone = formatPhoneForSMS(customerPhone)
    if (!formattedPhone) {
      console.warn(`[Order Status SMS] Numéro de téléphone invalide: ${customerPhone}`)
    ...
    if (order.fulfillment === 'delivery') {
      if (newStatus === 'preparing') {
        ...
        // Le livreur va chercher la commande au restaurant
        message = `Votre commande #${orderNumber} est prête. Le livreur va la chercher au restaurant et sera bientôt en route vers vous.`
      }
      // Note: Le message "livreur proche" sera géré par une API séparée basée sur la position GPS
    }
    ...
    if (!message) {
      console.log(`[Order Status SMS] Aucun message configuré pour fulfillment: ${order.fulfillment}, status: ${newStatus}`)
      return { success: false, reason: 'No message configured for this status transition' }
    }

    console.log(`[Order Status SMS] Envoi du SMS à ${formattedPhone} pour la commande #${orderNumber}, statut: ${newStatus}`)
    const result = await sendSMS(formattedPhone, message.trim())
    
    if (result.success) {
      console.log(`[Order Status SMS] SMS envoyé avec succès. SID: ${result.sid}`)
      return { success: true, sid: result.sid }
    } else {
      console.error(`[Order Status SMS] Échec de l'envoi du SMS: ${result.error}`)
      return { success: false, reason: result.error }
    }
  } catch (error) {
    console.error('[Order Status SMS] Erreur inattendue:', error)
